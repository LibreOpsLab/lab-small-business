#!/usr/bin/env bash
# Joins an Ubuntu Desktop/Server host to LAB.LOCAL via SSSD (not winbind — SSSD is the
...
realm join --client-software=sssd --os-name="Ubuntu" \
  --computer-ou="OU=Linux,OU=Workstations,OU=LAB,DC=lab,DC=local" \
  -U "${JOIN_USER}" "${DOMAIN}"
...
if [[ -f "${SCRIPT_DIR}/../templates/sssd.conf.j2" ]]; then
  ...
  sed -e 's/{{ *lab_domain[^}]*}}/lab.local/' \
      -e 's/{{ *lab_realm[^}]*}}/LAB.LOCAL/' \
...
log "Next: run ansible/playbooks/05-pki-trust.yml against this host so HTTPS to *.lab.local is trusted."
