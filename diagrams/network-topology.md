...
network (`VMnet8`) and a **LAN Segment** named `LAN-LAB` (`10.10.10.0/24`) that carries all lab
...
flowchart TB
    ...
    subgraph HOST["VMware Workstation Host (Windows 11)"]
        ...
        subgraph LAN_SEG["LAN — LAN Segment 'LAN-LAB', 10.10.10.0/24"]
            PFSENSE["pfSense\nWAN + LAN\n10.10.10.1/24"]
            DC["samba-dc01\nAD DC / DNS / KDC / NTP\n10.10.10.10"]
...
| Host           | Role                                      | Address                 |
| -------------- | ----------------------------------------- | ----------------------- |
| pfSense (LAN)  | Gateway / Firewall / DHCP / DNS forwarder | `10.10.10.1/24`         |
| samba-dc01     | AD DC, LDAP, Kerberos, DNS, NTP           | `10.10.10.10`           |
| docker01       | Docker Engine, reverse proxy, apps        | `10.10.10.20`           |
| authentik01    | Authentik IAM (bare host or VM)           | `10.10.10.30`           |
| linux-client01 | Ubuntu Desktop, SSSD domain member        | DHCP `10.10.10.100-199` |
| win-client01   | Windows 11, domain member                 | DHCP `10.10.10.100-199` |

DHCP pool `10.10.10.100–10.10.10.199` is served by pfSense; DHCP option 6 (DNS) points clients
at `10.10.10.10` so domain-joined machines resolve `lab.internal` correctly. See
